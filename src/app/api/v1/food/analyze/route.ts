...
import { estimateNutritionFromPhoto } from "@/lib/ai-utils";
...
export async function POST(request: NextRequest) {
  ...
  const aiResult = await estimateNutritionFromPhoto(uploadResult.signedUrl);
  if (aiResult.error || !aiResult.data) {
    return apiError("AI_FAILED", aiResult.error || "AI estimation failed", 500);
  }
